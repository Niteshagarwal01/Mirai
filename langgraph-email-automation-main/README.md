...
2. **Run the setup script:**
   ...
   This script will guide you through creating a virtual environment, installing dependencies, and setting up the necessary configuration files.
...
3. **Install the required packages:**
   ...
   > **Note:** The requirements include `protobuf==3.20.3` - this specific version is intentionally pinned to ensure compatibility with Chromadb.
   
   If you encounter issues during installation or running the application, please refer to the [Troubleshooting Guide](TROUBLESHOOTING.md).
...
4. **Set up environment variables:**
   ...
   GROQ_API_KEY=your_groq_api_key     # Optional but recommended (from groq.com)
   GEMINI_API_KEY=your_gemini_api_key # Required - get from Google AI Studio
   ```
   
   **Required API Keys:**
   - **GEMINI_API_KEY**: Required for both embeddings and as a backup LLM. Get from [Google AI Studio](https://ai.google.dev/).
   - **GROQ_API_KEY**: Optional but recommended for better performance. Get from [Groq](https://console.groq.com/keys).
...
### **Excel-based Marketing Campaigns**

- **Automate marketing outreach** using contact information from Excel files
- **Personalized email generation** tailored to recipient's industry, company size, and role
- **Track processed leads** to avoid duplicate emails
- **Configurable campaign settings** including email sending limits and delays

### **How to Run Marketing Campaigns**

1. **Create a contacts Excel file** with the following columns:
   - `company_name`: Name of the target company
   - `contact_person`: Name of the contact person
   - `role`: Role/position of the contact person
   - `email`: Email address
   - `industry`: Industry of the company (optional)
   - `company_size`: Size of the company (optional)

2. **Run the campaign script**:
   ```bash
   python run_marketing_campaign.py contacts.xlsx --limit 5 --delay 10
   ```
   - `--limit`: Maximum number of emails to send (optional)
   - `--delay`: Minutes between emails (default: 5)

3. **Create a template file** to get started:
   ```bash
   python create_contacts_template.py
   ```

4. **Reset processed leads** if needed:
   ```bash
...
To keep the repository clean and maintain optimal performance, use the provided cleanup script:

```sh
# Basic cleanup (removes __pycache__, clears processed data files, removes DB file)
python cleanup_repo.py

# Full cleanup (also removes the entire DB directory)
...
```sh
# Clear processed leads to restart marketing campaigns
python clear_processed_leads.py

# Create a fresh contacts template
python create_contacts_template.py
```

### Additional Resources

If you're using a Unix-like system (Linux/macOS), you can take advantage of the included Makefile:
